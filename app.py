...
def _ytdlp_download(url, file_id):
    ...
    try:
        ...
        # Video bo'lsa ffmpeg bilan H.264 ga konvert qilish
        if ext in ["mp4", "webm", "mov", "avi", "mkv"]:
            converted = f.rsplit(".", 1)[0] + "_conv.mp4"
            try:
                ...
                if os.path.exists(converted) and os.path.getsize(converted) > 10000:
                    os.remove(f)
                    f = converted
                    ext = "mp4"
                else:
                    # Konvert muvaffaqiyatsiz — asl faylni ishlatamiz
