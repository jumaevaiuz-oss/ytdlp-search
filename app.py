...
def get_cookiefile():
    ...
    # Railway ba'zan \n ni literal qilib saqlaydi — tuzatamiz
...
def search():
    ...
    try:
        ...
        if not result:
            return jsonify({"error": "Natija topilmadi"}), 400
...
def download_audio():
    ...
    ydl_opts = {
        "outtmpl": output_path,
        "format": "bestaudio/best",          # ← tuzatilgan format
        "quiet": True,
        "no_warnings": True,
        "max_filesize": MAX_SIZE_BYTES,
        "http_headers": {
...
@app.route("/health")
def health():
    return jsonify({"status": "ok", "cookies": bool(os.environ.get("YTDLP_COOKIES"))})
